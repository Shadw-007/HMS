...
$query="insert into  registration(roomno,bedspace,feespm,foodstatus,stayfrom,duration,course,schoolid,firstName,middleName,lastName,gender,contactno,emailid,egycontactno,guardianName,guardianRelation,guardianContactno,corresAddress,corresCIty,corresProvince,corresPincode,pmntAddress,pmntCity,pmnatetProvince,pmntPincode) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
$stmt = $mysqli->prepare($query);
$rc=$stmt->bind_param('iiiisisissssisississsisssi',$roomno,$bedspace,$feespm,$foodstatus,$stayfrom,$duration,$course,$schoolid,$fname,$mname,$lname,$gender,$contactno,$emailid,$emcntno,$gurname,$gurrelation,$gurcntno,$caddress,$ccity,$cprovince,$cpincode,$paddress,$pcity,$pprovince,$ppincode);
$stmt->execute();
...
<input type="radio" value="1" name="foodstatus"> With Food(ZMW 2000.00 Extra)
</div>
</div>	
...
<?php $query ="SELECT * FROM provinces";
$stmt2 = $mysqli->prepare($query);
$stmt2->execute();
$res=$stmt2->get_result();
while($row=$res->fetch_object())
...
<label class="col-sm-2 control-label">Province </label>
<div class="col-sm-8">
<select name="pprovince" id="pprovince"class="form-control" required> 
<option value="">Select Province</option>
<?php $query ="SELECT * FROM provinces";
$stmt2 = $mysqli->prepare($query);
$stmt2->execute();
